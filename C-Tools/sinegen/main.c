...
unsigned short wavetab[WAVETABLE_SIZE];

const unsigned short	sinetab[WAVETABLE_SIZE] = {
...
int i,k=0;
	...
	{
		if ( sinetab[i] <= HALF_DAC_RESOLUTION )
			wavetab[i] = sinetab[i] - HALF_DAC_RESOLUTION;
		else
			wavetab[i] = (HALF_DAC_RESOLUTION - sinetab[i])*-1;
	}
    printf("__attribute__((section(\".table\"))) const int16_t	sinetab[WAVETABLE_SIZE] = {\n",wavetab[i] );

	for(i=0;i<WAVETABLE_SIZE;i++)
	{
        printf("0x%04x, ",wavetab[i] );
        k++;
        if ( k == 8 )
        {
            k = 0;
